...
elseif (isset($_SESSION[$cookie]))
{
	if (isset($_REQUEST['operation']))
	{
		...
		elseif ($_REQUEST['operation'] == 'customers')
		{
			...
			$data = apiRequest($url, $token->access_token, 'GET', ['order'=>'Name','where'=>'IsCustomer is True'], ["xero-tenant-id: $tenantId"]);  
			if ($data['code'] == 200)
			{
		...
		elseif ($_REQUEST['operation'] == 'suppliers')
		{
			...
			# call the API - the xero API needs xero-tenant-id in the header
			$data = apiRequest($url, $token->access_token, 'GET', ['order'=>'Name','where'=>'IsSupplier is True'], ["xero-tenant-id: $tenantId"]);  
			if ($data['code'] == 200)
			{
				$table = contacts_summary($data['response']);
				print table_html($table);
